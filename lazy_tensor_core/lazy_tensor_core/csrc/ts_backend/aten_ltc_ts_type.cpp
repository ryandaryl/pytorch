...
at::Tensor LazyNativeFunctions::_to_copy(const at::Tensor & self,
    ...
    if (!lazy_self && device && device->type() == c10::kLazy) {
      ...
      device = c10::nullopt;
      torch::lazy::NodePtr node = torch::lazy::ReuseNode<ir::ops::ToCopy>(
          torch::lazy::OpKind(at::aten::_to_copy), lazy_self->GetIrValue(),
      ...
      if (!node) {
        ...
        node = torch::lazy::MakeNode<ir::ops::ToCopy>(
...
at::Tensor & LazyNativeFunctions::normal_(at::Tensor & self, double mean, double std, c10::optional<at::Generator> generator) {
    ...
    // LazyTensor lazy_self = GetLtcTensorOrCreateForWrappedNumber(self,
    // *device); std::vector<torch::lazy::Shape> shapes =
    // {torch::lazy::Shape(self.scalar_type(), self.sizes().vec())}; auto node =
    // torch::lazy::ReuseOrMakeNode<ir::ops::Normal>(
    //     torch::lazy::OpKind(at::aten::normal_), lazy_self.GetIrValue(), mean,
    //     std, std::move(shapes));
